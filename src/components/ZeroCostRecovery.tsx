...
export default function ZeroCostRecovery({selected, onClose, onSync}: {selected: BrokerPosition | null; onClose: () => void; onSync: () => void}) {
  ...
  useEffect(() => {
    let active = true
    setPreview(null);setReview(false);setAgreed(false);setError('');setPrincipal('');setPrice('');setFee('0')
    if (!selected) return
    setBusy(true)
    request('preview', {symbol:selected.symbol, account_id:selected.account_id || ''}).then(data => {
  ...
  async function prepare() {
    ...
    try {
      const data = await request('preview', {symbol:selected.symbol, account_id:selected.account_id || '', principal:Number(principal), price:Number(price), fee_buffer:Number(fee)})
  ...
  return <>
    {(jobs.length > 0 || jobsError) && <details className="mt-4 border-t border-border pt-3" open>
      <summary className="cursor-pointer text-sm font-medium">收回本金紀錄</summary>
      ...
      <div className="max-h-72 overflow-auto divide-y divide-border text-xs">
        {jobs.map(job => <details key={job.id} className="py-3">
          ...
          <p className="mt-2">限價 {money(job.price)} · 本金 {money(job.principal)}</p>
    ...
    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4" role="dialog" aria-modal="true" aria-labelledby="recover-title">
      <div className="w-full max-w-md max-h-[90vh] overflow-auto rounded-xl border border-border bg-card p-5 space-y-4">
        <div className="flex justify-between items-center"><h3 id="recover-title" className="font-semibold text-lg">{selected.symbol} · 收回本金</h3><button disabled={busy} onClick={onClose} aria-label="關閉" className="px-2">✕</button></div>
        ...
        {!review ? <>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="text-sm">費用預留 ($)<input className="mt-1 w-full rounded bg-secondary p-2" type="number" min="0" step="0.01" value={fee} onChange={e => setFee(e.target.value)} /></label>
          </div>
          <button disabled={busy || !Number(principal) || !Number(price)} className="w-full rounded bg-primary py-2 font-medium text-black disabled:opacity-40" onClick={prepare}>{busy ? '正在核對…' : '更新報價及預覽賣單'}</button>
        </> : preview && <>
          ...
          {preview.stop && <p className="text-xs text-warning">會先調整原止蝕股數，再提交賣單。掛單期間，待賣股數不受原止蝕保護；取消或部分成交後會重新核對。</p>}
          <label className="flex gap-2 text-sm"><input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} />我已核實本金、股數、限價{preview.stop ? '及止蝕調整' : ''}，確認真實賣出。</label>
