...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetchWithTimeout(`${process.env.PYTHON_API_URL || 'http://127.0.0.1:8000'}/api/pending-stops/retry`, {
      method: 'POST', cache: 'no-store',
      headers: { 'Content-Type': 'application/json', 'X-API-Key': process.env.API_SECRET || '' },
      body: JSON.stringify({ entry_order_id: body.entry_order_id }),
    })
